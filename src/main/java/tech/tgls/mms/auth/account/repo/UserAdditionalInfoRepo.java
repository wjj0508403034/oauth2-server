...
@Repository
public interface UserAdditionalInfoRepo extends
		CrudRepository<UserAdditionalInfo, Long> {

	@Query("select t from UserAdditionalInfo t where t.userId = ?1")
	List<UserAdditionalInfo> findUserInfosByUserId(Long userId);

	@Modifying
	@Query("delete from UserAdditionalInfo t where t.userId = ?1")
	void deleteUserInfosByUserId(Long userId);
}
